...
void saveGame() {
  ...
  if (!F) {
    renderMsgAndWait("Couldn't save the game", 3);
    fclose(F);
    return;
  }
  ...
  for (int i = 0; i < MAX_CURSE; ++i) num_curse += P->curse[i] != NULL;

  fwrite(&num_food, sizeof(int), 1, F);
  fwrite(&num_curse, sizeof(int), 1, F);

  for (int i = 0; i < MAX_FOOD; ++i)
    if (P->food[i]) fwrite(P->food[i], sizeof(Food), 1, F);
  for (int i = 0; i < MAX_CURSE; ++i)
    if (P->curse[i]) fwrite(P->curse[i], sizeof(Curse), 1, F);
